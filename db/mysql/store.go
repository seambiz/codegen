...
func (s *Store) Map(mapper func(row []sql.RawBytes), accumulator func(), args ...interface{}) error {
	rows, values, valuePointers, err := s.queryBegin(s.stmt.Query(), args...)
	if err != nil {
		return err
	}
...
func (s *Store) QueryJet(destPtr interface{}, args ...interface{}) error {
	log.Debug().Str("fn", "Store.QueryInto").Str("stmt", s.stmt.String()).Interface("args", args).Msg("sql")
	return qrm.Query(nil, s.db, s.stmt.Query(), args, destPtr)
}
