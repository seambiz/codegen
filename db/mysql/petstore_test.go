...
func TestPetUpsert(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherEqual))
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	defer db.Close()
	mock.
		ExpectExec("INSERT INTO fake_benchmark.pet ( `id` , `person_id` , `tag_id` , `species` ) VALUES ( '0' , '0' , '0' , '' ), ( '0' , '0' , '0' , '' ) ON DUPLICATE KEY UPDATE person_id = VALUES(person_id),tag_id = VALUES(tag_id),species = VALUES(species)").
		WillReturnResult(sqlmock.NewResult(0, 2))

	aff, err := NewPetStore(db).Upsert([]*codegen.Pet{{}, {}}...)
	if err != nil {
		t.Fatalf("SQL error '%s'", err)
	}
