...
func TestPersonUpsert(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherEqual))
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	defer db.Close()
	mock.
		ExpectExec("INSERT INTO fake_benchmark.person ( `id` , `name` ) VALUES ( '0' , '' ), ( '0' , '' ) ON DUPLICATE KEY UPDATE name = VALUES(name)").
		WillReturnResult(sqlmock.NewResult(0, 2))

	aff, err := NewPersonStore(db).Upsert([]*codegen.Person{{}, {}}...)
	if err != nil {
		t.Fatalf("SQL error '%s'", err)
	}
